...
FEV_pass_coefficients <- function(coefficient_array, trig,
  ...
  b_fev =
    ...
    BUE(ba_use, ba_use_bool) +             
    NSB(noc_s, noc_s_bool) +               
    DEE(dys_exer, DYS_EXER_CHECK(dys_exer)) +       
    DSE(sex, dys_exer, SEX_CHECK(sex,dys_exer)) +   
...
BINARY_CODE_FROM_INPUTS <- function(
  ...
  bc <- c(F1,F2,F3,F4,F5,F6,F7,F8,F9,F10,F11,F12,F13,F14,F15,F16,F17,F18) #DK - updated 2017-10-29 generates array of chars
  return(bc)
}
