...
setwd("~/Documents/RStudio projects1/20171111/FEV_7_7/")
...
server <- function(input, output, session) {
  ...
  #make lmer summary non-reactive --> it is only calculated when the user presses "Run Linear mixed-effects models" button
    output$lmer_summary <- renderPrint({
      ...
      if(file.exists(full_file_name) && is.null(GLOBAL_lmer_model_loaded_FLAG)){ #if model exists, but model has not been loaded, load the model from the file
        ptm <- proc.time()
        ...
        print('Execution time (if statement)')
        print(file_exists_exec_time)
      ...
      else if(file.exists(full_file_name) && !is.null(GLOBAL_lmer_model_loaded_FLAG)){ #if file exists and model has been loaded, then get the model from global variable
        if(input$lmer_summary_DropDownBox == 'Entire Summary'){
          GLOBAL_lmer_model_summary
        }
        else if(input$lmer_summary_DropDownBox == 'Coefficients'){
        ...
        file_exists_exec_time <- proc.time() - ptm
        print('Execution time (else statement)')
        print(file_exists_exec_time)
    ...
    output$plot_FEV1_decline <- renderPlot({
      ...
      file_name <- isolate(
        ...
        FEV_coeff_vector <- loadedCoefficients[c(1:28),] 
